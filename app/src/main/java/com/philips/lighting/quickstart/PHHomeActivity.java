...
public class PHHomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        phHueSDK.setAppName("IoTWeek3App");
        phHueSDK.setDeviceName(android.os.Build.MODEL);

        // Register the PHSDKListener to receive callbacks from the bridge.
        phHueSDK.getNotificationManager().registerSDKListener(listener);

        RecyclerViewClickListener<Integer, View> onRecyclerViewClickListener = new RecyclerViewClickListener<Integer, View>() {
            @Override
            public void onItemClicked(Integer item, View view) {
                PHAccessPoint accessPoint = (PHAccessPoint) adapter.getItem(item);
                connectToAccessPoint(accessPoint);
            }
        };
        adapter = new AccessPointListAdapter(phHueSDK.getAccessPointsFound(), onRecyclerViewClickListener);

        RecyclerView accessPointList = (RecyclerView) findViewById(R.id.bridge_list);
